...
describe("keyboard shortcuts", () => {
	...
	it("maps the QWERTY drum cluster to kit pieces", () => {
		...
		expect(resolveKeyboardShortcut({code: "KeyZ"})).toEqual({type: "triggerPad", pieceId: "kick", repeatable: false});
		expect(resolveKeyboardShortcut({code: "KeyF"})).toEqual({type: "triggerPad", pieceId: "ride", repeatable: false});
	});
	...
	it("keeps C as clap and K as the click toggle", () => {
		expect(resolveKeyboardShortcut({code: "KeyC"})).toEqual({type: "triggerPad", pieceId: "clap", repeatable: false});
	...
	it("reserves Shift+S for solo while plain S remains a drum trigger", () => {
		expect(resolveKeyboardShortcut({code: "KeyS"})).toEqual({type: "triggerPad", pieceId: "lowTom", repeatable: false});
		expect(resolveKeyboardShortcut({code: "KeyS", shiftKey: true})).toEqual({type: "toggleSolo", repeatable: false});
	...
	it("classifies navigation and volume controls as repeatable commands", () => {
		expect(resolveKeyboardShortcut({code: "ArrowLeft"})).toEqual({type: "nudgePlayback", direction: -1, repeatable: true});
		expect(resolveKeyboardShortcut({code: "ArrowRight"})).toEqual({type: "nudgePlayback", direction: 1, repeatable: true});
		expect(resolveKeyboardShortcut({code: "ArrowUp"})).toEqual({type: "focusLane", direction: -1, repeatable: true});
		expect(resolveKeyboardShortcut({code: "ArrowDown"})).toEqual({type: "focusLane", direction: 1, repeatable: true});
		expect(resolveKeyboardShortcut({code: "Equal"})).toEqual({type: "adjustVolume", delta: KEYBOARD_VOLUME_STEP, repeatable: true});
		expect(resolveKeyboardShortcut({code: "BracketLeft"})).toEqual({type: "adjustVolume", delta: -KEYBOARD_VOLUME_STEP, repeatable: true});
	});
