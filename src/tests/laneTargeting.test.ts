...
describe("active lane targeting", () => {
	...
	it("clamps arrow navigation at the first and last lanes", () => {
		expect(moveActiveLane(createActiveLane("kick", "pointer"), -1)).toEqual({pieceId: "kick", inputSource: "keyboard"});
		expect(moveActiveLane(createActiveLane("ride", "pointer"), 1)).toEqual({pieceId: "ride", inputSource: "keyboard"});
	});
});
