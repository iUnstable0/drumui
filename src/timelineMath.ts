...
export function getTimelineContentWidth(durationMs: number, viewportWidth: number, zoom: number): number {
	void durationMs;
	const safeViewportWidth = Math.max(MIN_CONTENT_WIDTH, Number.isFinite(viewportWidth) ? viewportWidth : MIN_CONTENT_WIDTH);
...
export function createTimelineScale(input: TimelineScaleInput): TimelineScale {
	...
	const viewportWidth = Math.max(MIN_CONTENT_WIDTH, Number.isFinite(input.viewportWidth) ? input.viewportWidth : MIN_CONTENT_WIDTH);
	...
	const overscanPx = Math.max(0, Number.isFinite(input.overscanPx) ? input.overscanPx ?? VISIBLE_OVERSCAN_PX : VISIBLE_OVERSCAN_PX);
	const visibleStartMs = xToTimeMs(Math.max(0, scrollLeft - overscanPx), durationMs, contentWidth);
	const visibleEndMs = xToTimeMs(Math.min(contentWidth, scrollLeft + viewportWidth + overscanPx), durationMs, contentWidth);
...
export function getTimelineHitPreviewToleranceMs(durationMs: number, contentWidth: number, markerWidthPx = TIMELINE_HIT_MARKER_WIDTH_PX): number {
	const safeDuration = Math.max(1, Number.isFinite(durationMs) ? durationMs : 1);
	const safeWidth = Math.max(MIN_CONTENT_WIDTH, Number.isFinite(contentWidth) ? contentWidth : MIN_CONTENT_WIDTH);
...
export function getFollowScrollLeft(timeMs: number, durationMs: number, contentWidth: number, viewportWidth: number, currentScrollLeft: number, edgePadding = 96): number {
	...
	if (playheadX > rightEdge) return clampTimelineScroll(playheadX - safeViewportWidth + edgePadding, contentWidth, safeViewportWidth);
...
export function formatTimelineTimestamp(milliseconds: number): string {
	...
	if (safeMilliseconds < 1000) return `0:${seconds.toString().padStart(2, "0")}.${tenths}`;
...
export function chooseTimelineTickIntervalMs(scale: TimelineScale, minLabelSpacingPx = MIN_LABEL_SPACING_PX): number {
	...
	const multiplier = BAR_MULTIPLIERS.find((value) => barWidth * value >= minLabelSpacingPx) ?? BAR_MULTIPLIERS[BAR_MULTIPLIERS.length - 1];
...
export function chooseTimelineGridIntervalMs(scale: TimelineScale): number {
	...
	const multiplier = BAR_MULTIPLIERS.find((value) => barWidth * value >= GRID_MIN_SPACING_PX) ?? BAR_MULTIPLIERS[BAR_MULTIPLIERS.length - 1];
	return scale.barMs * multiplier;
}
...
export function createTimelineGridLines(scale: TimelineScale): TimelineGridLine[] {
	...
	for (let timeMs = startMs; timeMs <= scale.visibleEndMs + 0.01; timeMs += intervalMs) {
		...
		const kind: TimelineGridLineKind = isBeat
			? beatIndex % TIMELINE_BEATS_PER_BAR === 0 ? "bar" : "beat"
			: "subbeat";
...
function snapLoopBoundaryMs(valueMs: number, durationMs: number, stepMs: number): number {
	if (valueMs >= durationMs - stepMs / 2) return durationMs;
...
export function dragTimelineLoopRange(input: TimelineLoopDragInput): TimelineLoopRange {
	...
	if (input.mode === "start") {
		...
		const startMs = clamp(snapLoopBoundaryMs(input.pointerMs, durationMs, stepMs), 0, Math.max(0, endMs - minLengthMs));
		return {startMs, endMs};
	}

	const startMs = clamp(originStartMs, 0, Math.max(0, durationMs - minLengthMs));
	const endMs = clamp(snapLoopBoundaryMs(input.pointerMs, durationMs, stepMs), startMs + minLengthMs, durationMs);
	return {startMs, endMs};
}
